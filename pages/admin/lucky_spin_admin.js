...
export default function LuckySpin() {
  ...
  const spining = () => {
    const awaredNumber = Math.floor(
      Math.random() * (playerShowList.length - 9)
    );
    console.log(awaredNumber);

    Array.from({ length: 9 }, (_, index) => index).forEach((idx) => {
      document
        .getElementById("spin-idx-" + idx)
        .classList.add("animate-move-down-" + idx);
    ...
    const timeoutPhase1 = setTimeout(() => {
      ...
      const timeoutPhase2 = setTimeout(() => {
        ...
        Array.from({ length: 9 }, (_, index) => index).forEach((idx) => {
          document
            .getElementById("spin-idx-" + idx)
            .classList.remove("animate-move-down-" + idx);
        ...
        <div className="flex flex-col justify-start items-center w-full h-full">
          <div className="flex flex-col w-full pt-5">
            ...
            <div className="flex w-full justify-between -mt-3 mb-1">
              <p className="font-[900] text-[#004599] text-[16px] text-left items-center h-6">
                Số người trực tuyến
              </p>
              <span className="flex gap-1">
                <p className="items-center text-center bg-[#3B88C3] text-white font-[900] rounded-md w-6 h-6">
                  {Math.floor(listPlayer.length / 100)}
                </p>
                <p className="items-center text-center bg-[#3B88C3] text-white font-[900] rounded-md w-6 h-6">
                  {Math.floor((listPlayer.length % 100) / 10)}
                </p>
                <p className="items-center text-center bg-[#3B88C3] text-white font-[900] rounded-md w-6 h-6">
                  {Math.floor((listPlayer.length % 100) % 10)}
                </p>
          ...
          <div className="w-full mb-12">
            <p className="font-[900] text-[#004599] uppercase text-[16px] text-center items-center">
            ...
            <div className="h-44 px-4 py-2 relative">
              <div>
                <div className="relative mt-1 before:block before:absolute before:-inset-0.5 before:bg-pink-500 before:bg-gradient-to-r before:from-[#003B93] before:to-[#00F0FF] before:rounded-md">
                  <button
                    type="button"
                    className="relative w-full cursor-default rounded-md border border-gray-300 bg-white p-2 shadow-sm border-none sm:text-sm outline-0"
                    aria-haspopup="listbox"
                    aria-expanded="true"
                    aria-labelledby="listbox-label"
                    onClick={toggleSelectMenu}
                  >
                    <p className="text-[#004599] font-bold text-base text-center w-full">
                      {listReward[rewardChosing].description}
                    </p>
                    <span className="pointer-events-none absolute inset-y-0 right-0 ml-3 flex items-center pr-2">
                      <svg
                        ...
                        viewBox="0 0 24 24"
                        fill="currentColor"
                        className="w-6 h-6 absolute right-2 origin-center fill-[#004599]"
                      >
                        <path
                          fillRule="evenodd"
                  ...
                  <ul
                    className="absolute z-10 mt-1 max-h-28 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm hidden"
                    tabIndex="-1"
                    role="listbox"
                    aria-labelledby="listbox-label"
                    aria-activedescendant="listbox-option-3"
                    id="selectRewardPopUp"
                  >
                    {listReward.map((reward, idx) => {
                      return (
                        <li
                          key={idx}
                          className="relative cursor-default select-none px-4 py-2 flex flex-row justify-between text-[#004599] font-normal hover:bg-[#40BEE5] hover:text-white hover:font-semibold"
                          id={"listbox-option-" + idx}
                          role="option"
                          style={{
                            background: idx === rewardChosing ? "#3B88C3" : "",
                            color: idx === rewardChosing ? "white" : "",
                            fontWeight: idx === rewardChosing ? "700" : "",
                          }}
                          onClick={() => {
                            setRewardChosing(idx), toggleSelectMenu();
                          }}
                        ...
                        >
                          ...
                          <span className="ml-3 block truncate">
                            Số lượng: {reward.quantity_remain}
                          </span>
                        </li>
                      );
                    })}
              ...
              <p className="w-full font-bold text-[#004599] text-center mt-2">
                Số lượng: {listReward[rewardChosing].quantity_remain}
              </p>
