...
function UserCountdownCheckin() {
  ...
  const getReward = query(
    ref(db, "event_rewards"),
    orderByChild("eventId"),
    equalTo(eventID)
  );
  ...
  useEffect(() => {
    const setOnlineStatus = (status) => {
      ...
      get(
        query(ref(db, "event_participants/" + user.participantId + "/status"))
      ).then((snapshot) => {
        if (snapshot.exists()) {
  ...
  const renderTitle = useMemo(() => {
    return (
      <div className="flex flex-col">
        <h1
          className={`font-[900] uppercase text-[#004599] text-[36px] text-center`}
        >
          {title}
        </h1>
        <h1
          className={`font-[900] uppercase text-[#004599] text-[18px] text-center mb-2`}
        >
          {trans.startAfter}
        </h1>
      </div>
  ...
  const renderCountdownTime = useMemo(() => {
    return (
      <div className="flex justify-center items-center max-w-xl w-4/5 mb-3">
        <div
          className="w-[65px] h-[80px] rounded-[10px] mr-1 text-white text-6xl flex justify-center items-center drop-shadow-lg"
        ...
        <div className="mr-1 text-6xl flex justify-center items-center">
          <span className="flex justify-center items-center text-[#3B88C3]">
            :
          </span>
        </div>
        <div
  ...
  const renderLine = useMemo(() => {
    return (
      <div className="max-w-xl w-4/5 mb-2 z-0">
        {" "}
        <Line />{" "}
      </div>
    );
  ...
  const renderTitle3 = useMemo(() => {
    return (
      <div className="flex flex-col">
        ...
        <h1
          className={`font-[900] uppercase text-[#004599] text-[18px] text-center mb-2`}
        >{`${trans.participant}: ${player}/${maxTicket}`}</h1>
      </div>
    );
  }, [player, maxTicket]);
  ...
  return (
    <section
      className={`h-screen w-screen flex flex-col items-center justify-center ${BG_COLOR}`}
    >
      {renderTitle}
      {renderCountdownTime}
      {renderTitle2}
