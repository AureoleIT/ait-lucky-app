...
export default function CurrentEventDetail({listPlayer, listReward}) {
    ...
    return (
        <div className="w-full absolute bottom-0 z-999 flex flex-col ease-in duration-200 overflow-hidden"  onClick={() => setExpand(!expand)}
            style={{height: !expand?"5rem":"100vh"}}>
                <div className="w-full h-full grow absolute rounded-t-2xl bottom-0 items-center z-999" onClick={() => setExpand(!expand)}>
                    {EventDetailExpand}
                </div>
        </div>
    );
